...
mod tests {
    ...
    fn f32_empty_returns_zero() {
        let itae = ITAE::<f32>::default();
        assert_eq!(itae.value(), 0.0_f32);
    }

    #[test]
    fn f32_time_weighted_absolute_average() {
        let mut itae = ITAE::<f32>::default();
        let state = first_state();
        itae.block(3.0, state); // n=1: acc += |3|*1 = 3
        itae.block(-2.0, state); // n=2: acc += |-2|*2 = 4
        itae.block(4.0, state); // n=3: acc += |4|*3 = 12
        // acc = 19, value = 19 / 3
        let expected = 19.0_f32 / 3.0;
        assert!((itae.value() - expected).abs() < 1e-6);
    }
    ...
    fn f32_block_returns_input_unchanged() {
        let mut itae = ITAE::<f32>::default();
        let state = first_state();
        assert_eq!(itae.block(-5.0, state), -5.0_f32);
        assert_eq!(itae.block(7.0, state), 7.0_f32);
    }
    ...
    fn f32_reset_clears_accumulator() {
        let mut itae = ITAE::<f32>::default();
        let state = first_state();
        itae.block(10.0, state);
        itae.block(-6.0, state);
        itae.reset();
        assert_eq!(itae.value(), 0.0_f32);
        itae.block(2.0, state); // n=1: acc = 2
        assert_eq!(itae.value(), 2.0_f32);
    }
    ...
    fn f64_empty_returns_zero() {
        let itae = ITAE::<f64>::default();
        assert_eq!(itae.value(), 0.0_f64);
    }

    #[test]
    fn f64_time_weighted_absolute_average() {
        let mut itae = ITAE::<f64>::default();
        let state = first_state();
        itae.block(1.0, state); // n=1: acc += 1*1 = 1
        itae.block(-2.0, state); // n=2: acc += 2*2 = 4
        itae.block(3.0, state); // n=3: acc += 3*3 = 9
        // acc = 14, value = 14 / 3
        let expected = 14.0_f64 / 3.0;
        assert!((itae.value() - expected).abs() < 1e-12);
    }
    ...
    fn f64_reset_clears_accumulator() {
        let mut itae = ITAE::<f64>::default();
        let state = first_state();
        itae.block(8.0, state);
        itae.block(-4.0, state);
        itae.reset();
        assert_eq!(itae.value(), 0.0_f64);
    }
    ...
    fn complex_f32_empty_returns_zero() {
        let itae = ITAE::<Complex<f32>> {
            acc: Complex::new(0.0, 0.0),
            n: 0,
        };
        assert_eq!(itae.value(), Complex::new(0.0_f32, 0.0));
    }
    ...
    fn complex_f32_divides_accumulated_by_count() {
        ...
        assert_eq!(itae.value(), Complex::new(3.0_f32, 4.0));
    }
    ...
    fn complex_f64_empty_returns_zero() {
        let itae = ITAE::<Complex<f64>> {
            acc: Complex::new(0.0, 0.0),
            n: 0,
        };
        assert_eq!(itae.value(), Complex::new(0.0_f64, 0.0));
    }
    ...
    fn smatrix_f32_empty_returns_zero() {
        let itae = ITAE::<SMatrix<f32, 3, 1>> {
            acc: SMatrix::<f32, 3, 1>::new(1.0, 2.0, 3.0),
            n: 0,
        };
        assert_eq!(itae.value(), SMatrix::<f32, 3, 1>::zeros());
    }
    ...
    fn smatrix_f32_column_vector_divides_by_count() {
        ...
        assert_eq!(itae.value(), SMatrix::<f32, 3, 1>::new(1.0, 2.0, 3.0));
    }

    #[test]
    fn smatrix_f32_square_divides_by_count() {
        let itae = ITAE::<SMatrix<f32, 2, 2>> {
            acc: SMatrix::<f32, 2, 2>::new(3.0, 6.0, 9.0, 12.0),
            n: 3,
        };
        assert_eq!(itae.value(), SMatrix::<f32, 2, 2>::new(1.0, 2.0, 3.0, 4.0));
    }
    ...
    fn smatrix_f64_empty_returns_zero() {
        let itae = ITAE::<SMatrix<f64, 3, 1>> {
            acc: SMatrix::<f64, 3, 1>::new(1.0, 2.0, 3.0),
            n: 0,
        };
        assert_eq!(itae.value(), SMatrix::<f64, 3, 1>::zeros());
    }
    ...
    fn smatrix_f64_column_vector_divides_by_count() {
        ...
        assert_eq!(itae.value(), SMatrix::<f64, 3, 1>::new(1.0, 2.0, 3.0));
    }

    #[test]
    fn smatrix_f64_square_divides_by_count() {
        let itae = ITAE::<SMatrix<f64, 2, 2>> {
    ...
    fn smatrix_complex_f32_empty_returns_zero() {
        let itae = ITAE::<SMatrix<Complex<f32>, 3, 1>> {
        ...
        assert_eq!(itae.value(), SMatrix::<Complex<f32>, 3, 1>::zeros());
    }
    ...
    fn smatrix_complex_f64_empty_returns_zero() {
        let itae = ITAE::<SMatrix<Complex<f64>, 2, 2>> {
        ...
        assert_eq!(itae.value(), SMatrix::<Complex<f64>, 2, 2>::zeros());
    }
